      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-200 flex items-center gap-1">
            <Lock className="w-3 h-3 text-blue-600" /> 256-Bit Encrypted Gateway
          </span>
          <span className="text-[10px] text-emerald-700 font-semibold flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-radar" /> Razorpay Test Rail
          </span>
        </div>
        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Express Checkout</h1>
        <p className="text-xs text-slate-500 mt-0.5">Enter delivery destination and complete instant settlement via Razorpay modal</p>
      </div>

      <form onSubmit={handleInitiatePayment} className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        {/* Shipping Information Form */}
        <div className="lg:col-span-2 p-6 rounded-3xl card-premium border border-slate-200 space-y-5 shadow-sm">
          <div className="border-b border-slate-200 pb-3">
            <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Shipping Details</h3>
            <p className="text-xs text-slate-500 mt-0.5">Physical delivery address across India</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div>
              <label className="text-slate-600 block mb-1.5 font-medium">Full Name</label>
              <input
                type="text"
                required
                value={shippingAddress.fullName}
                onChange={(e) => setShippingAddress({ ...shippingAddress, fullName: e.target.value })}
                className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-slate-300 text-slate-900 focus:outline-none focus:border-blue-500 transition-colors font-medium shadow-inner"
              />
            </div>

            <div>
              <label className="text-slate-600 block mb-1.5 font-medium">Phone Number (UPI/SMS Alerts)</label>
              <input
                type="text"
                required
                value={shippingAddress.phone}
                onChange={(e) => setShippingAddress({ ...shippingAddress, phone: e.target.value })}
                className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-slate-300 text-slate-900 focus:outline-none focus:border-blue-500 transition-colors font-mono shadow-inner"
              />
            </div>

            <div className="sm:col-span-2">
              <label className="text-slate-600 block mb-1.5 font-medium">Delivery Address</label>
              <input
                type="text"
                required
                value={shippingAddress.addressLine}
                onChange={(e) => setShippingAddress({ ...shippingAddress, addressLine: e.target.value })}
                className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-slate-300 text-slate-900 focus:outline-none focus:border-blue-500 transition-colors shadow-inner"
              />
            </div>

            <div>
              <label className="text-slate-600 block mb-1.5 font-medium">City</label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="text-slate-600 block mb-1.5 font-medium">Pincode</label>
              <input
                type="text"
                required
        ...
        <div className="p-6 rounded-3xl card-premium border border-slate-200 space-y-5 shadow-sm">
          <div className="border-b border-slate-200 pb-3">
            <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Payment Summary</h3>
            <p className="text-xs text-slate-500 mt-0.5">Razorpay Test Mode</p>
          </div>
          ...
          <div className="space-y-2 text-xs pt-3 border-t border-slate-200">
            <div className="flex justify-between text-slate-600">
              <span>Subtotal</span>
              <span className="text-slate-900 font-mono">₹{cartSubtotal.toLocaleString('en-IN')}</span>
            </div>
            {discountAmount > 0 && (
            ...
            <div className="flex justify-between items-baseline text-sm font-bold text-slate-900 pt-2 border-t border-slate-200">
          ...
          <button
            ...
            className="w-full py-4 px-4 rounded-xl font-black text-xs bg-gradient-to-r from-blue-600 via-blue-700 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white flex items-center justify-center gap-2 shadow-lg shadow-blue-600/25 transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
          >
